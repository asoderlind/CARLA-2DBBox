...
def auto_annotate(
    ...
    if json_path is not None:
        ...
        vehicle_class = []
    filtered_out, removed_out, _, _ = filter_occlusion_bbox(
        bounding_boxes_2d,
        vehicles,
        camera,
        depth_img,
...
def auto_annotate_debug(
    ...
    if json_path is not None:
        ...
        vehicle_class = []
    filtered_out, removed_out, depth_area, depth_show = filter_occlusion_bbox(
        bounding_boxes_2d,
        vehicles,
        camera,
        depth_img,
...
def filter_occlusion_1p(
    vehicles_list, v_transform, v_transform_s, sensor, depth_img, depth_margin=2.0
):
    camera_k_matrix = get_camera_intrinsic(sensor)
    CAM_W = int(sensor.attributes["image_size_x"])
    CAM_H = int(sensor.attributes["image_size_y"])
    pos_x_y_z = v_transform_s.T
    pos_y_minus_z_x = np.concatenate(
        [pos_x_y_z[1, :], -pos_x_y_z[2, :] - 0.0, pos_x_y_z[0, :]]
    )
    img_pos = np.transpose(np.dot(camera_k_matrix, pos_y_minus_z_x))
    camera_pos = np.concatenate(
        [img_pos[:, 0] / img_pos[:, 2], img_pos[:, 1] / img_pos[:, 2], img_pos[:, 2]],
        axis=1,
    )
...
def filter_occlusion_bbox(
    ...
    for v, vs, bbox in zip(vehicles, v_transform_s, bounding_boxes):
        ...
        if depth_margin < 0:
            depth_margin = (
                v.bounding_box.extent.x**2 + v.bounding_box.extent.y**2
            ) ** 0.5 + 0.25
...
def extract_depth(depth_img):
    ...
    depth_meter = (
        np.array(depth_img.raw_data).reshape((depth_img.height, depth_img.width, 4))[
            :, :, 0
        ]
        * 1000
        / 255
